;====
; Manages a VBlank callback in RAM that will be jumped to when a VBlank
; interrupt occurs
;====

;====
; RAM
;====
.ramsection "zest.vblank.ram" slot zest.mapper.RAM_SLOT
    ; Either a jp to the callback or a ret instruction
    zest.vblank.ram.jpToCallbackOrRet:    db

    ; The user specific VBlank callback
    zest.vblank.ram.callback:             dw
.ends

;====
; (Private) Sets the given VBlank callback address in RAM
; @in   stack{0}    pointer to the callback address
;====
.section "zest.vblank._setCallback" free
    zest.vblank._setCallback:
        ex (sp), hl   ; get callback address from stack; preserve hl

        push af
            ld a, $c3   ; jp instruction
            ld (zest.vblank.ram.jpToCallbackOrRet), a
            ld a, (hl)
            ld (zest.vblank.ram.callback), a
            inc hl
            ld a, (hl)
            ld (zest.vblank.ram.callback + 1), a
        pop af

        inc hl  ; point to address after the callback data
        ex (sp), hl   ; restore hl; push return address to stack

        ret
.ends

;====
; Resets any custom VBlank callback so it just returns
;
; @clobbers af
;====
.macro "zest.vblank.resetCallback"
    ld a, $c9   ; ret instruction
    ld (zest.vblank.ram.jpToCallbackOrRet), a
.endm

;====
; Calls the current custom VBlank callback
;====
.macro "zest.vblank.invokeCallback"
    call zest.vblank.ram.jpToCallbackOrRet
.endm

;====
; Defines an inline VBlank callback routine that will be jumped over but called
; on the next VBlank.
;====
.macro "zest.vblank.start"
    call zest.vblank._setCallback
    .dw zest.vblank.start\@  ; define data after the call

    ; jp over the inline callback
    jp zest.vblank.end\@

    ; Label for the callback following from the macro call
    zest.vblank.start\@:
.endm

.macro "zest.vblank.end"
    ; Return from the callback
    ret

    zest.vblank.end\@:
.endm
